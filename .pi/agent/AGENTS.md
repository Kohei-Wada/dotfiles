...
Web access is the `web_search` and `fetch_content` tools from pi-web-access,
pointed at the private SearXNG on thor. Neither reaches a third-party search API.

- For anything time-sensitive, current, or that you are not certain of, call `web_search` FIRST, then open the top result with `fetch_content`. Do not answer factual questions from memory alone.
...
- Prefer `web_search` for open-ended lookups; call `fetch_content` directly only on a URL you already know is correct.
- Search result snippets are NOT enough to answer from. Always open the top result with `fetch_content` and answer from the page body. If the page does not contain the answer, say so — do not fill the gap from memory.
